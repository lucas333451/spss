...
- `results/research/analysis-2/task5/overall/<raw|qc>/analysis2_task5_spss_polynomial_contrasts.csv` + `.md` + `analysis2_task5_wwr_profile_means.csv` + `analysis2_task5_subject_means_<DV>.csv` + `task5_spss_polynomial_figures/*.png`（新增：整体样本版、结果口径更贴近原始 SPSS 工作流的三水平 WWR Polynomial Contrasts；如果目标是替代原 SPSS 步骤，优先看这里）
- `results/research/analysis-2/task5/<experience|sportfreq>_group_only/<raw|qc>/analysis2_task5_spss_polynomial_contrasts.csv` + `.md` + `analysis2_task5_wwr_profile_means.csv` + `analysis2_task5_subject_means_<DV>.csv` + `task5_spss_polynomial_figures/*.png`（新增：只分组、不分 Round 的中间层结果；对 QC 后约 24 人的小样本更适合作为分组主结论入口）
- `results/research/analysis-2/task5/<experience|sportfreq>/<raw|qc>/analysis2_task5_spss_polynomial_contrasts.csv` + `.md` + `analysis2_task5_wwr_profile_means.csv` + `analysis2_task5_subject_means_<DV>.csv` + `task5_spss_polynomial_figures/*.png`（新增：Group × Round 的细分跟进版；用于进一步看 ExperienceGroup 或 SportFreqGroup 在不同 Round 下的分层结果，并附带 PNG 可视化）
