...
def build(results_root: Path, out: Path, max_rows: int) -> None:
    ...
    # 5) Groups
    lines.append("## 5) 人群差异（两种二分优先，4类补充）")
    lines.append("\n### 5.1 二分：SportFreq")
    ...
    lines.append("\n### 6.3 场景内人群差异（C0 单独）")
    lines.append(csv_md(r / "research/scene_group_comparisons_c0.csv", max_rows))
    lines.append("\n### 6.4 场景内人群差异（C1 单独）")
    lines.append(csv_md(r / "research/scene_group_comparisons_c1.csv", max_rows))
    lines.append("\n---\n")
