...
def _candidates(results_root: Path) -> list[tuple[str, str, str]]:
    ...
    return [
        ...
        (
            ...
            "research/task2_core_imm_suite/analysis2_core_imm_suite_figures/task2_s_model_aic.png",
            "Model selection evidence for S-items (AIC).",
        ),
        (
            "Fig03_task2_s_effects_forest",
            "research/task2_core_imm_suite/analysis2_core_imm_suite_figures/task2_s_top_effects_forest.png",
            "Key fixed-effect directions and confidence intervals (S-items).",
        ),
        (
            "Fig04_task2_b_model_aic",
            "research/task2_core_imm_suite/analysis2_core_imm_suite_figures/task2_b_model_aic.png",
            "Model selection evidence for B-items (C1-only).",
        ),
        (
            "Fig05_task2_b_effects_forest",
            "research/task2_core_imm_suite/analysis2_core_imm_suite_figures/task2_b_top_effects_forest.png",
        ...
        (
            ...
            "research/task1_stage_gap/analysis2_scene_stage_gap_figures/scene_S01.png",
    ...
    for label, rel, purpose in _candidates(root):
        ...
        if not src.exists():
            ...
            if label == "Fig08_task1_scene_example":
                pool = sorted((root / "research/task1_stage_gap/analysis2_scene_stage_gap_figures").glob("scene_*.png"))
                if pool:
                    src = pool[0]
                else:
                    missing.append(rel)
