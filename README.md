...
- `results/research/analysis-2/task5/<experience|sportfreq>/<raw|qc>/analysis2_task5_spss_polynomial_contrasts.csv` + `.md` + `analysis2_task5_wwr_profile_means.csv` + `analysis2_task5_subject_means_<DV>.csv` + `task5_spss_polynomial_figures/*.png` (NEW: split-cell SPSS-aligned repeated-measures polynomial contrasts for 3 WWR levels; includes profile means and PNG visualizations for grouped follow-up analysis)
