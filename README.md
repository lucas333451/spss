...
- `results/research/analysis-2/task5/overall/<raw|qc>/analysis2_task5_spss_polynomial_contrasts.csv` + `.md` + `analysis2_task5_wwr_profile_means.csv` + `analysis2_task5_subject_means_<DV>.csv` + `task5_spss_polynomial_figures/*.png` (NEW: overall-sample SPSS-aligned repeated-measures polynomial contrasts for 3 WWR levels; preferred first stop when you want results closest to the original SPSS workflow)
- `results/research/analysis-2/task5/<experience|sportfreq>_group_only/<raw|qc>/analysis2_task5_spss_polynomial_contrasts.csv` + `.md` + `analysis2_task5_wwr_profile_means.csv` + `analysis2_task5_subject_means_<DV>.csv` + `task5_spss_polynomial_figures/*.png` (NEW: group-only layer without Round splitting; recommended intermediate layer before moving to finer stratification, especially for the QC sample size around 24)
- `results/research/analysis-2/task5/<experience|sportfreq>/<raw|qc>/analysis2_task5_spss_polynomial_contrasts.csv` + `.md` + `analysis2_task5_wwr_profile_means.csv` + `analysis2_task5_subject_means_<DV>.csv` + `task5_spss_polynomial_figures/*.png` (NEW: Group × Round follow-up layer; includes profile means and PNG visualizations for finer grouped interpretation)
...
- `results/ipq/ipq_report_zh.md` (NEW: IPQ report)
